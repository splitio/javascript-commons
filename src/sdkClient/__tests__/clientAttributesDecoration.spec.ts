...
// @ts-expect-error
const client: any = clientAttributesDecoration(loggerMock, clientMock);
...
describe('ATTRIBUTES DECORATION / evaluation', () => {
  ...
  test.each([
    ['getTreatment', 'split'],
    ['getTreatments', ['split']],
    ['getTreatmentWithConfig', 'split'],
    ['getTreatmentsWithConfig', ['split']],
    ['getTreatmentsByFlagSet', 'set'],
    ['getTreatmentsWithConfigByFlagSet', 'set'],
    ['getTreatmentsByFlagSets', ['set']],
    ['getTreatmentsWithConfigByFlagSets', ['set']]
  ])('Evaluation attributes logic and precedence / %s', (method, param) => {
    // If the same attribute is “cached” and provided on the function, the value received on the function call takes precedence.
    expect(client[method]('key', param)).toEqual(undefined); // Nothing changes if no attributes were provided using the new api
    expect(client[method]('key', param, { func_attr_bool: true, func_attr_str: 'true' })).toEqual({ func_attr_bool: true, func_attr_str: 'true' }); // Nothing changes if no attributes were provided using the new api
    ...
    expect(client[method]('key', param, { func_attr_bool: true, func_attr_str: 'true' })).toEqual({ func_attr_bool: true, func_attr_str: 'true' }); // Function attributes has precedence against api ones
    expect(client[method]('key', param, null)).toEqual({ func_attr_bool: false }); // API attributes should be kept in memory and use for evaluations
    expect(client[method]('key', param, { func_attr_str: 'true' })).toEqual({ func_attr_bool: false, func_attr_str: 'true' }); // API attributes should be kept in memory and use for evaluations
    ...
    expect(client[method]('key', param, { func_attr_bool: true, func_attr_str: 'true', func_attr_number: 1 })).toEqual({ func_attr_bool: true, func_attr_str: 'true', func_attr_number: 1 }); // Function attributes has precedence against api ones
    expect(client[method]('key', param, null)).toEqual({ func_attr_bool: false, func_attr_str: 'false' }); // If the getTreatment function is called without attributes, stored attributes will be used to evaluate.
    expect(client[method]('key', param)).toEqual({ func_attr_bool: false, func_attr_str: 'false' }); // If the getTreatment function is called without attributes, stored attributes will be used to evaluate.
    expect(client.clearAttributes()).toEqual(true);
  });
