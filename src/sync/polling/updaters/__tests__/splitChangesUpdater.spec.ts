...
import { splitChangesUpdaterFactory, parseSegments, computeMutation, getRegisteredSegments } from '../splitChangesUpdater';
...
import { splitWithAccountTTAndUsesSegments } from '../../../../storages/__tests__/testUtils';
...
test('compute splits mutation', () => {
  const splitFiltersValidation = { queryString: null, groupedFilters: { bySet: [], byName: [], byPrefix: [] }, validFilters: [] };

  let splitsMutation = computeMutation([activeSplitWithSegments, archivedSplit] as ISplit[], splitFiltersValidation);
  ...
  splitsMutation = computeMutation([testFFSetsAB, test2FFSetsX] as ISplit[], splitFiltersValidation);

  expect(splitsMutation.added).toEqual([testFFSetsAB, test2FFSetsX]);
  expect(splitsMutation.removed).toEqual([]);
});

test('compute splits mutation with filters', () => {
  ...
  let splitsMutation = computeMutation([testFFSetsAB], splitFiltersValidation);
  ...
  splitsMutation = computeMutation([testFFRemoveSetB], splitFiltersValidation);

  expect(splitsMutation.added).toEqual([testFFRemoveSetB]);
  expect(splitsMutation.removed).toEqual([]);

  // fetching existing test feature flag removed from set B
  splitsMutation = computeMutation([testFFRemoveSetA], splitFiltersValidation);

  expect(splitsMutation.added).toEqual([]);
  expect(splitsMutation.removed).toEqual([testFFRemoveSetA]);

  // fetching existing test feature flag removed from set B
  splitsMutation = computeMutation([testFFEmptySet], splitFiltersValidation);
  ...
  splitsMutation = computeMutation([testFFSetsAB], splitFiltersValidation);

  expect(splitsMutation.added).toEqual([]);
  expect(splitsMutation.removed).toEqual([testFFSetsAB]);

  splitsMutation = computeMutation([test2FFSetsX, testFFEmptySet], splitFiltersValidation);
...
test('get registered segments', async () => {
  const storage = {
    splits: {
      getAll: () => Promise.resolve([splitWithAccountTTAndUsesSegments])
    },
    rbSegments: {
      getAll: () => Promise.resolve([rbsWithExcludedSegment])
    }
  };

  // @ts-expect-error
  const segments = await getRegisteredSegments(storage);

  expect(segments.sort()).toEqual(['C', 'employees']);
});
