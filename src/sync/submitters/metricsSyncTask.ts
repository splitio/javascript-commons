...
export function countsSyncTaskFactory(
  ...
  return submitterSyncTaskFactory(postMetricsCounters, countsCache, metricsRefreshRate, 'count metrics', latencyTracker, fromCache<number>('delta'));
}
...
export function latenciesSyncTaskFactory(
  ...
  // don't retry metrics.
  return submitterSyncTaskFactory(postMetricsLatencies, latenciesCache, metricsRefreshRate, 'latency metrics', latencyTracker, fromCache<number[]>('latencies'));
}
