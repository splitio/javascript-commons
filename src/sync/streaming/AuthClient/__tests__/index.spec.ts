...
test('authenticate / success in node (200)', done => {
  ...
  fetchMock.getOnce(authUrl + '/auth', (url, opts) => {
    // @ts-ignore
...
test('authenticate / success in browser (200)', done => {
  ...
  fetchMock.getOnce(authUrl + '/auth?users=emi%40split.io&users=maldo%40split.io', (url, opts) => {
    // @ts-ignore
    expect(opts.headers['Authorization']).toBe(`Bearer ${authorizationKey}`); // auth request must contain Authorization header with config authorizationKey
    return { status: 200, body: authDataResponseSample };
  });
...
test('authenticate / bad request in browser due to no user keys (400)', done => {
  ...
  fetchMock.getOnce(authUrl + '/auth', { status: 400, body: '"no user specified"' });
...
test('authenticate / Invalid credentials (401)', done => {
  ...
  fetchMock.getOnce(authUrl + '/auth', { status: 401, body: '"Invalid credentials"' });
...
test('authenticate / HTTP error (other than 401)', done => {
  ...
  fetchMock.getOnce(authUrl + '/auth', { status: NOT_OK_STATUS_CODE, body: 'some error message' });
...
test('authenticate / Network error (e.g., timeout)', done => {
  ...
  fetchMock.getOnce(authUrl + '/auth', { throws: new TypeError('Network error') });
...
test('authenticate / Error parsing token', done => {
  ...
  fetchMock.getOnce(authUrl + '/auth', { status: 200, body: { pushEnabled: true, token: jwtSampleInvalid } });
  fetchMock.getOnce(authUrl + '/auth', { status: 200, body: { pushEnabled: true, token: jwtSampleNoChannels } });
  fetchMock.getOnce(authUrl + '/auth', { status: 200, body: { pushEnabled: true, token: jwtSampleNoIat } });
