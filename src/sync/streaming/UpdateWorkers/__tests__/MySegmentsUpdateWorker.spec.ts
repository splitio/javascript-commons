...
describe('MySegmentsUpdateWorker', () => {
  ...
  test('put', async () => {
    ...
    expect(mySegmentsSyncTask.execute).toHaveBeenLastCalledWith(['some_segment'], true, undefined); // synchronizes MySegments with given segmentList
    mySegmentsSyncTask.__resolveMySegmentsUpdaterCall(); // fetch success
    await new Promise(res => setTimeout(res, 10));
    ...
    expect(mySegmentsSyncTask.execute).toHaveBeenLastCalledWith(['other_segment'], true, undefined);
