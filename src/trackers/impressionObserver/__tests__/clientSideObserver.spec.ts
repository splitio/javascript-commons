import { hashImpression32, impressionObserverCSFactory } from '../impressionObserverCS';
import { generateImpressions } from './testUtils';

test('Hasher 32 / Impression Hasher Works', () => {
  const imp1 = {
    feature: 'someFeature',
    keyName: 'someKey',
    changeNumber: 123,
  ...
  expect(hashImpression32(imp1)).toBe(hashImpression32(imp2));

  // Different feature
  imp2.feature = 'someOtherFeature';
  expect(hashImpression32(imp1)).not.toBe(hashImpression32(imp2));
...
test('Hasher 32 / Impression Hasher Does Not Crash', () => {
  const imp1 = {
    feature: 'someFeature',
    keyName: 'someKey',
    changeNumber: 123,
...
test('Client-side (Browser JS) / Impression Observer Basic Functionality', () => {
  const observer = impressionObserverCSFactory();
