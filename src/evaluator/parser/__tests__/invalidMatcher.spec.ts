...
test('PARSER / handle invalid matcher as control', async function () {
  ...
  expect(evaluation.label).toBe('unsupported matcher type'); // track invalid as an exception
});
...
test('PARSER / handle invalid matcher as control (complex example)', async function () {
  ...
  for (let ev of [ev1, ev2, ev3]) {
    ...
    expect(ev.label).toBe('unsupported matcher type'); // track invalid as an exception
  }
});
