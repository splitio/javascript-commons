import { ERROR_NULL, ERROR_EMPTY, ERROR_INVALID, WARN_API_KEY, SETTINGS_LB } from '../../logger/constants';
...
export function validateApiKey(log: ILogger, maybeApiKey: any): string | false {
  ...
  if (maybeApiKey == undefined) { // eslint-disable-line eqeqeq
    log.error(ERROR_NULL, [SETTINGS_LB, item]);
  } else if (isString(maybeApiKey)) {
    if (maybeApiKey.length > 0)
      apiKey = maybeApiKey;
    else
      log.error(ERROR_EMPTY, [SETTINGS_LB, item]);
  } else {
    log.error(ERROR_INVALID, [SETTINGS_LB, item]);
  }
