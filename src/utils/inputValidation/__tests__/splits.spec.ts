...
describe('INPUT VALIDATION for Split names', () => {
  ...
  test('Should return the provided array if it is a valid splits names array without logging any errors', () => {
    ...
    expect(validateSplitMock).toBeCalledTimes(validArr.length); // Should have validated each value independently.
    expect(loggerMock.error).not.toBeCalled(); // Should not log any errors on the collection.
  ...
  test('Should strip out any invalid value from the array', () => {
    ...
    expect(loggerMock.warn).not.toBeCalled(); // It should have not logged any warnings for the collection.
  });
});
