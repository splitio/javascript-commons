...
// PRIVATE FUNCTIONS
// -----------------
function _x64Add(m: number[], n: number[]): [number, number] {
  //
...
function _x64Multiply(m: number[], n: number[]): [number, number] {
  //
  // Given two 64bit ints (as an array of two 32bit ints) returns the two
...
function _x64LeftShift(m: [number, number], n: number) {
  //
  // Given a 64bit int (as an array of two 32bit ints) and an int
  // representing a number of bit positions, returns the 64bit int (as an
...
function _x64Xor(m: number[], n: number[]): [number, number] {
  //
  // Given two 64bit ints (as an array of two 32bit ints) returns the two
...
function _x64Fmix(h: number[]): number[] {
  //
...
export function hash128(str: string, seed?: number): string {
  return isX64 ? hash128x64(UTF16ToUTF8(str), (seed as number) >>> 0) : hash128x86(UTF16ToUTF8(str), (seed as number) >>> 0);
}
